...
export type RegisterEventsCallback = () => Promise<void> | void;
...
export interface AuthorizeService {
  ...
  signinPassword(params: RopcContext): Promise<AuthenticationResult>;
  registerEvents(cb: RegisterEventsCallback): void;
  cleanupEvents(cb: RegisterEventsCallback): void;
}
...
export class ManagedLogger {
  ...
  log(level: LogLevel, message: string): void {
    if ((level == LogLevel.Trace && this.trace) || (level == LogLevel.Debug && this.debug)) {
...
class OidcAuthorizeService implements AuthorizeService {
  ...
  async getAccessToken(request?: AccessTokenRequestOptions): Promise<AccessTokenResult> {
    ...
    if (hasValidAccessToken(user) && hasAllScopes(request, user.scopes)) {
      this.debug(`Valid access token present expiring at '${getExpiration(user.expires_in ?? 0).toISOString()}'`);
      return {
        status: AccessTokenResultStatus.Success,
        token: {
          grantedScopes: user.scopes,
          expires: getExpiration(user.expires_in ?? 0),
          value: user.access_token,
        },
      };
    } else {
      try {
        const parameters = request && request.scopes ? { scope: request.scopes.join(" ") } : undefined;
        ...
        this.debug(`Provisioned an access token expiring at '${getExpiration(newUser.expires_in ?? 0).toISOString()}'`);

        const result = {
          status: AccessTokenResultStatus.Success,
          token: {
            grantedScopes: newUser.scopes,
            expires: getExpiration(newUser.expires_in ?? 0),
  ...
  async signinPassword(context: RopcContext) {
    this.trace("signinPassword", context);

    try {
      await this._userManager.clearStaleState();
      const user = await this._userManager.signinResourceOwnerCredentials(this.createRopcArguments(context));
      this.debug("Password sign in succeeded");

      return this.success(user && user.state);
    } catch (error) {
      const message = this.getExceptionMessage(error);
      this.debug(`Password sign in failed '${message}'.`);
      return this.error(message);
    }
  }
  ...
  private eventPrinterHandler(message: string) {
    return () => this.debug(message);
  }

  registerEvents(cb: RegisterEventsCallback) {
    this.debug("register events for user");

    this._userManager.events.addAccessTokenExpired(this.eventPrinterHandler("access token expired event"));
    this._userManager.events.addSilentRenewError(this.eventPrinterHandler("silent renew error"));

    this._userManager.events.addAccessTokenExpired(cb);
    this._userManager.events.addSilentRenewError(cb);
  }

  cleanupEvents(cb: RegisterEventsCallback) {
    this.debug("cleanup user events");

    this._userManager.events.removeAccessTokenExpired(this.eventPrinterHandler("access token expired event"));
    this._userManager.events.removeSilentRenewError(this.eventPrinterHandler("silent renew error"));

    this._userManager.events.removeAccessTokenExpired(cb);
    this._userManager.events.removeSilentRenewError(cb);
  }
  ...
  private getExceptionMessage(error: any) {
    ...
    function isOidcError(error: any): error is SigninResponse & SignoutResponse {
  ...
  private createRopcArguments(params: RopcContext): SigninResourceOwnerCredentialsArgs {
    return {
      extraTokenParams: {
        scope: params?.scopes ? params.scopes.join(" ") : undefined,
...
export class AuthenticationService {
  ...
  public static signinPassword(context: RopcContext) {
    return AuthenticationService.instance.signinPassword(context);
  }
  ...
  private static eventCallback(dotNetHelper: any) {
    return async () => await dotNetHelper.invokeMethodAsync("EventsCallback");
  }

  public static registerErrorEvents(dotNetHelper: any) {
    AuthenticationService.instance.registerEvents(AuthenticationService.eventCallback(dotNetHelper));
  }

  public static cleanupErrorEvents(dotNetHelper: any) {
    AuthenticationService.instance.cleanupEvents(AuthenticationService.eventCallback(dotNetHelper));
  }
  ...
  private static createUserManagerCore(finalSettings: UserManagerSettings) {
    ...
    const userManager = new UserManager({ ...finalSettings, userStore: new WebStorageStateStore({ store }) });
...
window.AuthenticationService2 = AuthenticationService;
